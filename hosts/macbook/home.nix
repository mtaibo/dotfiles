{ pkgs, lib, ... }: {
  ...
  home.activation.setWallpaper = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    WALLPAPER_SRC=${../../assets/hypr/wallpaper.jpg}
    $DRY_RUN_CMD mkdir -p "$WALLPAPER_DIR"
    $DRY_RUN_CMD cp "$WALLPAPER_SRC" "$WALLPAPER_DIR/wallpaper.jpg"
    $DRY_RUN_CMD osascript -e "tell application \"System Events\" to set picture of every desktop to POSIX file \"$WALLPAPER_DIR/wallpaper.jpg\"" 2>/dev/null || true
  '';
